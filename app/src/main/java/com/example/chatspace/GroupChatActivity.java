...
public class GroupChatActivity extends AppCompatActivity {
 ...
 // private TextView displayTextMessages; // 2. Text View To Display Data
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        chatRv.addOnItemTouchListener(
                new RecyclerItemClickListener(getApplicationContext(), new RecyclerItemClickListener.OnItemClickListener() {
                    @Override
                    public void onItemClick(View view, int position) {
                        if (position != RecyclerView.NO_POSITION){
                            coordinatesFromMessage = String.valueOf(groupMessageArrayList.get(position).getMessage());
                            ClipboardManager manager = (ClipboardManager) getSystemService(CLIPBOARD_SERVICE);
                            ClipData clipData = ClipData.newPlainText("text", coordinatesFromMessage);
